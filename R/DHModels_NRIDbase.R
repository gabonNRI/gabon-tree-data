...
####################################################
heightfit <- function(hdat, sub.dat, E_Value){
    ...
    E_Value <- E_Value
    sub.dat <- sub.dat
    #Plot_name <- unique(sub.dat$Plot.Code)
...
##### EQ18:Equation 8 in Huang et al. (1992 and 2000): H = 1.3 + (a*D)/(D+1) + b*D, where D = DBH
    ...
    equation <- formula(y ~ 1.3 + a*D/(D+1) + b*D, showEnv = F)
...
##### EQ32: The Mitscherlisch model (Ngomanda et al., 2014): H = a - b * exp(-cD), where D = DBH
    ...
    res32 <- results.fun(fit = Eq32, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a32, b32, c32))
...
##### Model comparison
 ...
 predht <- predht.fun(res = res.fin, sub.dat)
  full.list <- res.fin
   full.list$predht <- predht
   full.list$Xequ <- as.character(res.fin$equation)[3]
    full.list
}
