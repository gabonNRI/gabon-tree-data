...
BAfun <- function(D){0.00007854*D^2 * 10000}  # Multiply by 10000 to express in cm2

####################################################
heightfit <- function(hdat, sub.dat){
    ...
    #E_Value <- E_Value
    sub.dat <- sub.dat
    Plot_name <- unique(hdat$Plot.Code)

##### Calculating RSD for Uncertainity & Organizing Output

   results.fun <- function (fit, equation, Hmean, D, error, PredHt, coef){

  	 AIC <- AIC_RSS(n = length(Hmean), length(fit$par), error)
  	 RMSE <- rmse(error)

  	 RSD <- sqrt(sum(error^2)/(length(Hmean)-2))
      Xstand <- (D - mean(D))^2
      SEM <- RSD * sqrt(1/length(Hmean) + Xstand/sum(Xstand))
       res.df <- cbind(D, Hmean, PredHt, Residuals = error, SEM = SEM)
      return(list(equation = equation, RSD = RSD, vars = c(coef, AIC = AIC, RMSE = RMSE),  res = res.df))

  }


 plot.fun <- function(res, hdat, data_labels){
    a <- res$vars["a"]
    b <- res$vars["b"]
    c <- res$vars["c"]
    d <- res$vars["d"]
    equ.fun <- function(D) eval(parse(text = res$equ))
     ...
     ggplot(hdat, aes(x = D, y = Hmean)) + geom_point(colour="grey", size = 3) +
        xlab("Tree diameter, cm") + ylab("Tree height, m") + theme_bw() +
        theme(legend.position = "none") +
        stat_function(fun = equ.fun, colour = "darkgreen", size = 1) +
        geom_text(data = data_labels, aes(x = time , y = value, label = label), size = 4)
...
##### EQ1: Linear model: H = a + b*D, where D = DBH
 ...
 startvals1 <- list(a = 2, b = 1)
    ...
    res1 <- results.fun(fit = Eq1, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a1, b1))
...
##### EQ2: Quadratic model, Eq. 27 in Table 2 of page 128 (Huang et al., 2000): H = a + b*D + c*D^2, where D = DBH
    ...
    res2 <- results.fun(fit = Eq2, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a2, b2, c2))
...
##### EQ3: Linear models on log scale (Log~Log): H=exp(a+b*ln(D)), where D = DBH (Eq2 of page 8617 (Djomo et al., 2010; Molto et al., 2013))
    ...
    res3 <- results.fun(fit = Eq3, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a3, b3))
...
##### EQ4: Quadratic model on log scale: Ln(H) = a + b * ln(D) + c * ln(D^2) (Djomo et al., 2010)
    ...
    res4 <- results.fun(fit = Eq4, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a4, b4, c4))
...
##### Log-linear eq.1 in page 8617 (Fang and Bailey, 1998; Molto et al., 2013)
    ...
    res5 <- results.fun(fit = Eq5, equation = equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a5, b5))
...
##### EQ6: Simplified (two-parameter) Weibull equation: H = a*(1-exp(-D/b)), where D=DBH, page 8617 (Molto et al., 2013)
    ...
    Fit_nls6 <- function(x) {
          a <- x[1]
          b <- x[2]
          t1 <- (a * (1-exp(I(-D/b))))              #Log-linear eq.1 in page 8617 (Fang and Bailey, 1998; Molto et al., 2013)
          t1 <- sum((Hmean-t1)^2)
          return(t1)}
    ...
    res6 <- results.fun(fit = Eq6, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a6, b6))
...
##### Equation 13 of Huang et al., 1992; Lewis et al., 2009; Feldpausch et al., 2012; Mitchard et al., 2014)
    ...
    res7 <- results.fun(fit = Eq7, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a7, b7, c7))
...
##### EQ8: Michaelis-Menten equation: H = a*D /(b+D), in page 8618 (Molto et al., 2013; Equation 3 in Huang et al. (1992))
    ...
    res8 <- results.fun(fit = Eq8, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a8, b8))
...
##### Start points were estimated from Ngomanda et al.(2014)
    ...
    res9 <- results.fun(fit = Eq9, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a9, b9))
...
##### EQ10:Model 3: Ln(H) = a + b/D or H = a * exp(b/D), where D=DBH (Djomo et al., 2010; Equation 6 in Huang et al. (1992))
    ...
    res10 <- results.fun(fit = Eq10, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a10, b10))
...
##### Start points were for Balsam poplar of Equation 12 in page 1289 (Huang et al., 1992)
    ...
    res11 <- results.fun(fit = Eq11, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a11, b11, c11))
...
##### Start points were for Balsam poplar of Equation 12 in page 1289 (Huang et al., 1992)
    ...
    res12 <- results.fun(fit = Eq12, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a12, b12, c12))
...
##### Start points were for Black williows in page 173 (Colbert et al., 2002)
    ...
    res13 <- results.fun(fit = Eq13, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a13, b13, c13))
...
##### Start points were for Balsam poplar of Equation 12 in page 1289 (Huang et al., 1992)
    ...
    res14 <- results.fun(fit = Eq14, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a14, b14, c14))
...
##### EQ15: Equation 4 in Huang et al.(1992 and 2000): H = 1.3 + a * (1 - exp(-bD)), where D = DBH
    ...
    res15 <- results.fun(fit = Eq15, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a15, b15))
...
##### EQ16:Equation 5 in Huang et al. (1992 and 2000): H = 1.3 + D^2 /(a + bD)^2, where D = DBH
    ...
    res16 <- results.fun(fit = Eq16, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a16, b16))
...
##### EQ17: Equation 7 in Huang et al. (1992 and 2000): H = 1.3 + 10^a*(D^b), where D = DBH
    ...
    res17 <- results.fun(fit = Eq17, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a17, b17))
...
##### EQ18:Equation 8 in Huang et al. (1992 and 2000): H = 1.3 + (a*D)/(D+1) + b*D, where D = DBH
    ...
    res18 <- results.fun(fit = Eq18, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a18, b18))
...
#####  EQ19:Equation 9 in Huang et al. (1992 and 2000): H = 1.3 + a*(D/(D+1))^b, where D = DBH
    ...
    res19 <- results.fun(fit = Eq19, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a19, b19))
...
##### EQ20:Equation 10 in Huang et al. (1992 and 2000): H = 1.3 + exp(a + b*D^c), where D = DBH
    ...
    res20 <- results.fun(fit = Eq20, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a20, b20, c20))
...
##### EQ21: Equation 11 in Huang et al. (1992 and 2000): H = 1.3 + a/(1+b*exp(-c*D)), where D = DBH
    ...
    res21 <- results.fun(fit = Eq21, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a21, b21, c21))
...
##### EQ22: Equation 14 in Huang et al. (1992 and 2000): H = 1.3 + a*exp(-b*exp(-c*D)), where D = DBH   (For Eqs. 12 and 13 in Huang et al. (1992 and 2002), please see the above eqs 14 and 7 in the file.)
    ...
    res22 <- results.fun(fit = Eq22, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a22, b22, c22))
...
##### EQ23: Equation 15 in Huang et al. (1992) and Eq. 16 in Huang et al. (2000): H = (1.3^b + (c^b-1.3^b)*(1-exp(-a*D))/(1-exp(-a*100)))^(1/b)
    ...
    res23 <- results.fun(fit = Eq23, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a23, b23, c23))
...
##### Start points were for Balsam poplar of Equation 12 in page 1289 (Huang et al., 1992)
    ...
    res24 <- results.fun(fit = Eq24, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a24, b24, c24))
...
##### EQ25: Equation 17 in Huang et al. (1992) and Eq. 18 in Huang et al. (2000): H = 1.3 + a *D^(b*D^-c), where D = DBH
    ...
    res25 <- results.fun(fit = Eq25, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a25, b25, c25))
...
##### EQ26: Equation 18 in Huang et al. (1992) and Eq. 20 in Huang et al. (2000): H = 1.3 + a*exp(b/(D+c)), where D = DBH
    ...
    res26 <- results.fun(fit = Eq26, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a26, b26, c26))
...
##### EQ27: Equation 19 in Huang et al.(1992) and Eq.19 in Huang et al. (2000): H = 1.3 + a/(1 + b^-1 * D^-c), where D = DBH
    ...
    res27 <- results.fun(fit = Eq27, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a27, b27, c27))
...
##### EQ28: Equation 20 in Huang et al. (1992) and Eq. 21 in Huang et al. (2000): H = 1.3 + a*(1 - b*exp(-c*D))^d, where D = DBH
    ...
    res28 <- results.fun(fit = Eq28, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a28, b28, c28, d28))
...
##### Start values were for white spruce (Table 3 in page 131 of Huang et al. (2000))
    ...
    res29 <- results.fun(fit = Eq29, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a29, b29))
...
##### EQ30: Equation 24 in Huang et al. (2000): H = 1.3 + a * D^b * exp(-c*D), where D = DBH. Start values were for white spruce (Table 3 in page 131 of Huang et al. (2000))
    ...
    res30 <- results.fun(fit = Eq30, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a30, b30, c30))
...
#    res31 <- results.fun(fit = Eq31, equation, Hmean = Hmean, D = D, error = error, PredHt = PredHt, coef = c(a31, b31, c31))

##### EQ31: The Mitscherlisch model (Ngomanda et al., 2014): H = a - b * exp(-cD), where D = DBH
    startvals31 <- list(a = 44.7, b = 42.3, c = 0.026)
    Fit_nls31 <- function(x) {
          a <- x[1]
          b <- x[2]
          c <- x[3]
...
##### Model comparison
    ...
    AIC.df <- matrix(nrow = 31, ncol = 2, dimnames = list(c(), c("Equ", "AIC")))
 ...
 gp <- plot.fun(res = res.fin, hdat = hdat, data_labels)
 print(gp)

 predht.fun <- function(res){
 	a <- res$vars["a"]
    b <- res$vars["b"]
    c <- res$vars["c"]
    d <- res$vars["d"]
 	 equ.fun <- function(D) eval(parse(text = res$equ))
 	  ph <- equ.fun(sub.dat$D)
 }

 predht <- predht.fun(res = res.fin)
  full.list <- res.fin
